...
namespace Ontec.Core.Application.Consumption.Handler.Queries
{
    namespace Ontec.Core.Application.Consumption.Handler.Queries
    {
        public class LatestHandler : IRequestHandler<GetConsumptionMastersQuery, ConsumptionMastersDto>
...

        {
            ...
            private readonly ILogger<LatestHandler> _logger;
            ...
            public LatestHandler(IUserRepository userRepository
            ...
            public async Task<ConsumptionDashboardDto> Handle(GetConsumptionDashboardQuery request, CancellationToken cancellationToken)
            {
                ...
                switch (request.ConsumptionCylceTypeId)
                {
                    ...
                    case 2: // Weekly
                            ...
                            // ✅ ISO-aligned range
                            endDate = ISOWeek.ToDateTime(isoYear, isoWeek, DayOfWeek.Monday)
                                         .AddDays(7);

                            startDate = endDate.AddDays(-42);

                            dailyConsumption *= 7;
                            guageDailyConsumption *= 7;

                            // ✅ ISO week X-axis (calendar correct)
                ...
                if (meterResult != null)
                {
                    ...
                    // 🔥 Align to end of ISO week (Sunday)
                    endDate = ISOWeek.ToDateTime(isoYear, isoWeek, DayOfWeek.Monday)
                                 .AddDays(7);
                    ...
                    if (intervalReadingData.Count > 0)
                    {
                        if (request.ConsumptionCylceTypeId == (int)ConsumptionCylceEnum.Daily)
                        {
                            ...
                            var dailyTotals = hourlyBase
                                .GroupBy(x => x.Day.Date)   // 🔥 IMPORTANT: .Date
                                .ToDictionary(
                                    g => g.Key,
                            ...
                            for (var day = rangeStart; day <= rangeEnd; day = day.AddDays(1))
                            {
                                intervalWise.Add(new CaclucationInterval
                                {
                                    ...
                                    ActualConsumption =
                                        dailyTotals.TryGetValue(day.Date, out var val) ? val : 0
                                });
                            }
                        }
                        ...
                        if (request.ConsumptionCylceTypeId == (int)ConsumptionCylceEnum.Weekly)
                        {
                            ...
                            //    var weekKey = $"{week.Key.IsoYear}-W{week.Key.IsoWeek:D2}";

                            //    XAxisArray.Add(weekKey);
                            ...
                            if (request.ConsumptionCylceTypeId == (int)ConsumptionCylceEnum.Weekly)
                            {
                                ...
                                {
                                    var weekStart = ISOWeek.ToDateTime(
                                        ISOWeek.GetYear(cursor),
                                        ISOWeek.GetWeekOfYear(cursor),
                                        DayOfWeek.Monday
                                    );

                                    var weekKey = $"{ISOWeek.GetYear(weekStart)}-W{ISOWeek.GetWeekOfYear(weekStart):D2}";
            ...
            //    // Generate ALL weeks in data range
            ...
            //   // 2.Get unique ISO weeks from data days(calendar-driven, like dashboard)
            //    var weeks = dailyTotals.Keys
            //        .GroupBy(d => new
            //        {
            //            IsoYear = ISOWeek.GetYear(d),
            ...
            //            // 4. Sum exactly 7 calendar days (matches dashboard weekly logic)
            //            double weekTotal = Enumerable.Range(0, 7)
            //                .Sum(i =>
            //                {
            //                    var day = weekStart.AddDays(i);
            ...
            //    // 1️⃣ Pick reference day (latest available day)
            //    var referenceDay = hourlyBase.Max(x => x.Day).Date;

            //    // 2️⃣ Get ISO week for that day
            ...
            //    // 3️⃣ Build daily totals dictionary
            //    var dailyTotals = hourlyBase
            //        .GroupBy(x => x.Day.Date)
            //        .ToDictionary(
            //            g => g.Key,
            ...
            //    // 5️⃣ DAYS + HOURS (calendar driven)
            //    for (int i = 0; i < 7; i++)
            //    {
            //        var day = weekStart.AddDays(i);
    ...
    List<Reading> intervalReadingData,
    string weekKey = null   // e.g. "2026-W04" (null for default)
)
            {
                var data = new List<ConsumptionDataDTO>();

                if (intervalReadingData == null || !intervalReadingData.Any())
                    return data;

                // 1️⃣ Build hourly base (your existing trusted logic)
                var hourlyBase = BuildHourlyBase(intervalReadingData);

                if (!hourlyBase.Any())
                    return data;

                // 2️⃣ Decide ISO week (SINGLE source of truth)
                int isoYear, isoWeek;

                if (!string.IsNullOrEmpty(weekKey))
                {
                    // 👉 Drill-down from Weekly
                    var parts = weekKey.Split("-W", StringSplitOptions.RemoveEmptyEntries);
                    isoYear = int.Parse(parts[0]);
                    isoWeek = int.Parse(parts[1]);
                }
                else
                {
                    // 👉 Default Daily view → latest available week
                    var referenceDay = hourlyBase.Max(x => x.Day);
                    isoYear = ISOWeek.GetYear(referenceDay);
                    isoWeek = ISOWeek.GetWeekOfYear(referenceDay);
                }

                // 3️⃣ ISO week boundaries (calendar correct)
                var weekStart = ISOWeek.ToDateTime(isoYear, isoWeek, DayOfWeek.Monday);
                var weekEnd = weekStart.AddDays(7); // EXCLUSIVE

                string resolvedWeekKey = $"{isoYear}-W{isoWeek:D2}";

                // 4️⃣ Filter data to that ISO week
                hourlyBase = hourlyBase
                    .Where(x => x.Day >= weekStart && x.Day < weekEnd)
                    .ToList();

                // 5️⃣ Daily totals dictionary (calendar-safe)
                var dailyTotals = hourlyBase
                    .GroupBy(x => x.Day.Date)
                    .ToDictionary(
                        g => g.Key,
                ...
                // 6️⃣ WEEK ROOT NODE (optional but useful)
                data.Add(new ConsumptionDataDTO
                {
                    Arg = resolvedWeekKey,
                    StartDate = weekStart,
                ...
                // 7️⃣ DAYS + HOURS (calendar driven, ALWAYS 7 days)
                for (int i = 0; i < 7; i++)
                {
                    var day = weekStart.AddDays(i);
                    var dayKey = day.ToString("dd MMM");
