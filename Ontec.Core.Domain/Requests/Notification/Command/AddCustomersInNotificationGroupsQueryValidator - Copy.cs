...
{
    public class AddCustomersInNotificationGroupsQueryValidator : AbstractValidator<AddCustomersInNotificationGroupsQuery>
    {
        public AddCustomersInNotificationGroupsQueryValidator(INotificationRepository notificationRepository,
                                                            IUserRepository userRepository,IWorkContext workContext)
        {
            ...
            RuleFor(m => m).CustomAsync(async (model, context, cancellationToken) =>
            {
                ...
                foreach (var usrId in model.UserIds)
                {
                    if (usrId > 0)
                    {
                        var isExist = await userRepository.IsUserIdExist(usrId).ConfigureAwait(false);
                        if (!isExist)
                        {
                            context.AddFailure(nameof(AddCustomersInNotificationGroupsQuery.UserIds), string.Format(CommonConstants.NotExist, nameof(AddCustomersInNotificationGroupsQuery.UserIds)));
                        }
                    }
                }
