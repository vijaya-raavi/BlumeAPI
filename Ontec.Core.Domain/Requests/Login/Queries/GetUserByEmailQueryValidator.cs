...
namespace Ontec.Core.Domain.Requests.Login.Queries
{
    public class GetUserByEmailQueryValidator : AbstractValidator<GetUserByEmailQuery>
    {
                                ...
                                IUserBlockService _userBlockService)
        {
            ...
            RuleFor(x => x).CustomAsync(async (model, context, cencellation) =>
            {
                bool selfRegistered = await _userRepository.GetUserRegisteredStatus(model.Email, model.CompanyId).ConfigureAwait(false);
                if (!selfRegistered)
                {
                    context.AddFailure(nameof(GetUserByEmailQuery.Email), string.Format(CommonConstants.NotRegistered, nameof(GetUserByEmailQuery.Email)));
                }
