...
{
    public class BulkUploadRequestCommandValidator : AbstractValidator<BulkUploadRequestCommand>
    {
        public BulkUploadRequestCommandValidator(IUserRepository _userRepository,
                                                ...
                                                IDocumentRepository _documentRepository)
        {
            ...
            var dto = new List<MeterAgreementEntry>();
            ...
            RuleFor(m => m).CustomAsync(async (model, context, cancellationToken) =>
            {
                ...
                var meterUnitPairs = model.UploadBulkUsers
                           .Where(x => x.UserProperties != null)
                           .SelectMany(x => x.UserProperties)
                           .Where(p => p.Meters != null)
                           .SelectMany(p => p.Meters.Select(m => new
                           {
                ...
                var semaphore = new SemaphoreSlim(5); // max 5 concurrent API calls — tune as needed
                var agreementEntries = new ConcurrentBag<MeterAgreementEntry>();

                var tasks = meterUnitPairs.Select(async m =>
                {
                    await semaphore.WaitAsync();
                    try
                    {
                        var meterUrl = _masterApiSetting.BaseUrl + _masterApiSetting.MeterNumberApi
                                       + "?meter.meterNum=" + m.MeterNumber.ToUpper() + "&paging=(limit)(5)(offset)(0)";
                        ...
                        {
                            foreach (var item in meterResult.Data)
                            {
                                if (item.CustomerAgreement != null && !string.IsNullOrEmpty(item.CustomerAgreement.Id))
                                {
                                    agreementEntries.Add(new MeterAgreementEntry
                                    {
                                        UnitNumber = m.UnitNumber,
                                        MeterNumber = m.MeterNumber,
                                        CustomerAgreementId = item.CustomerAgreement.Id.ToString()
                                    });
                                }
                            }
                        }
                ...
                dto.AddRange(agreementEntries);
                var conflicts = dto
                                .Where(x => !string.IsNullOrEmpty(x.CustomerAgreementId))
                                .GroupBy(x => x.CustomerAgreementId)
                                .Where(g => g.Select(x => x.UnitNumber).Distinct().Count() > 1);
                ...
                foreach (var group in conflicts)
                {
                    ...
                    {
                        context.AddFailure(
                            $"Meter Number[{entry.MeterNumber}]",
                            $"Customer agreement id '{group.Key}' is linked to multiple units: {string.Join(", ", group.Select(x => x.UnitNumber).Distinct())}. This meter is mapped to unit '{entry.UnitNumber}'.");
                    }
                }
            ...
            if (user.UserProperties?.Any() == true)
            {
                foreach (var property in user.UserProperties)
                {
                    ...
                    if (property.Meters?.Any() == true)
                    {
                        ...
                        {
                            await ValidateMeter(meter, context, _meterRepository, _propertyRepository, _masterApiConnectService, _masterApiSetting, _configurationRepository, workContext, property.UnitNumber, dto);
                        }
                    }
                }
        ...
        // ── User Validation ──────────────────────────────────────────────────
        private async Task ValidateUser(BulkUsers user, ValidationContext<BulkUploadRequestCommand> context, IUserRepository userRepository, IUserRoleRepository _userRoleRepository, ICommunicationRepository _communicationRepository, IDocumentRepository _documentRepository)
        {

            if (user.Title == 0)
            {
                context.AddFailure($"Title [{user.Title}]", "Title is required.");
            }
            ...
            else
            {
                ...
                if (!Regex.IsMatch(user.Mobile, "^[0-9]{7,14}$"))
                {
            ...
            if (!string.IsNullOrEmpty(user.FirstName) && !Regex.IsMatch(user.FirstName, "^[a-zA-Z '-]+$"))
            {
                context.AddFailure($"First Name [{user.FirstName}]", "Invalid first name.");
            }
            if (!string.IsNullOrEmpty(user.LastName) && !Regex.IsMatch(user.LastName, "^[a-zA-Z '-]+$"))
            {
                context.AddFailure($"Last Name [{user.FirstName}]", "Invalid last name.");
            }
            // Check email exists
            ...
            if (user.Title > 0)
            {
                ...
                {
                    context.AddFailure($"TitleId [{user.Title}]", "Invalid title");
                }
            }
            if (user.UserDocument != null && user.UserDocument.DocumentType != "")
            {
                var docTypes = await _documentRepository.GetDocumentTypeMasters().ConfigureAwait(false);
                var docTypesName = docTypes.Select(s => s.Name.ToLower()).ToList();

                bool isExist = docTypesName.Contains(user.UserDocument.DocumentType.ToLower());
                if (!isExist)
                {
                ...
                int docTypesId = docTypes.ToList().Where(s => s.Name.ToLower().Equals(user.UserDocument.DocumentType.ToLower())).Select(t => t.Id).FirstOrDefault();

                if (user.UserDocument != null && docTypesId == (int)DocumentTypeEnum.SouthAfricanID)
                {
                    bool isLuhnDigit = false;
                    if (!string.IsNullOrEmpty(user.UserDocument.DocNumber))
                    {
                        string doc = user.UserDocument.DocNumber.Trim();
                        ...
                        if (!isLuhnDigit)
                        {
                            context.AddFailure($"DocNumber [{user.UserDocument.DocNumber}]", string.Format(CommonConstants.InValid, nameof(UpdateUserCommand.DocumentNumber).SplitPascalCase()));
                        }
                        else
                        {
                            if (user.Title == (int)Title.Mrs || user.Title == (int)Title.Ms)
                            {
                                ...
                                {
                                    context.AddFailure($"DocNumber [{user.UserDocument.DocNumber}]", string.Format(CommonConstants.SAID, nameof(UpdateUserCommand.DocumentNumber).SplitPascalCase()));
                                }
                                 if (!Regex.IsMatch(user.UserDocument.DocNumber, "^[0-9]{2}[0-9]{2}[0-9]{2}[0-4][0-9]{3}[0-1]{1}[8]{1}[0-9]{1}$"))
                               // if (!Regex.IsMatch(user.UserDocument.DocNumber, "^[0-9]{2}(0[1-9]|1[0-2])(0[1-9]|[12][0-9]|3[01])[0-4][0-9]{3}[0-1][8][0-9]$"))
                                {
                                    context.AddFailure($"DocNumber [{user.UserDocument.DocNumber}]", string.Format(CommonConstants.SAID, nameof(UpdateUserCommand.DocumentNumber).SplitPascalCase()));
                                }
                            }
                            if (user.Title == (int)Title.Mr)
                            {
                               ...
                               // if (!Regex.IsMatch(user.UserDocument.DocNumber, "^[0-9]{2}(0[1-9]|1[0-2])(0[1-9]|[12][0-9]|3[01])[5-9][0-9]{3}[0-1][8][0-9]$"))
                                {
                                    context.AddFailure($"DocNumber [{user.Email}]", string.Format(CommonConstants.SAID, nameof(UpdateUserCommand.DocumentNumber).SplitPascalCase()));
                                }
                            }
                        }
                    }
        ...
        private async Task ValidateProperty(
            ...
            int companyId,
            ValidationContext<BulkUploadRequestCommand> context,
        ...
        private async Task ValidateMeter(
            ...
            IWorkContext workContext, string unitNumber, List<MeterAgreementEntry> dto)
        {
            ...
            if (meterResult != null && meterResult.Data.Count() > 0)
            {
                try
                {
                    ...
                    if (meter.MeterTypeId == 0)
                    {
                        meter.MeterTypeId = utility.id;
                    }
            ...
            //if (!isMeterTypeExist)
            //    context.AddFailure($"Meter - Utility type id[{meter.MeterNumber}]", "Utility type id does not exist.");
            ...
            if (meterTypes != null && meterTypes.Any(t => t.id.Equals(meter.MeterTypeId)))
            {
                ...
                else if (Convert.ToDouble(minDailyTarget) != 0 && Convert.ToDouble(maxDailyTarget) != 0)
                {
                    ...
                    {
                        context.AddFailure($"Meter-{meter.MeterNumber} - DailyTargetConsumption[{meter.TargetConsumption}]", $"Daily target must be from {minDailyTarget} {unitOfMeasure} to {maxDailyTarget} {unitOfMeasure}.");
                    }
                }
            }
            ...
            if (meter.ContractDocumentUrl != null)
            {
                ...
                {
                    context.AddFailure($"Meter  - ContractDocument[{meter.MeterNumber}{meter.ContractDocumentUrl.FileName}]", $"Allowed file formats (pdf) with {defaultFileSize}MB.");
                }

                // ── Contract End Date ────────────────────────────────────────
                if (!string.IsNullOrEmpty(meter.ContractEndDate))
                {
                    if (DateTime.TryParseExact(meter.ContractEndDate, "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime contractDate))
                    {
